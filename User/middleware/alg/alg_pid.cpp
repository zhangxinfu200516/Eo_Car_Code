...
void Class_PID::Increase_Init(float __K_P, float __K_I, float __K_D,float __Out_Max,float __D_T, float __Dead_Zone)
{
    K_P = __K_P;
    K_I = __K_I;
    K_D = __K_D;
...
void Class_PID::TIM_Adjust_PID_Increase_PeriodElapsedCallback()
{
    // P输出
    float p_out = 0.0f;
    // I输出
    ...
    d_out = K_D * (error - 2.0f * Pre_Error + Pre_Pre_Error) / D_T;
