...
public:
    ...
    inline void OLED_WR_Byte(uint8_t dat,uint8_t cmd);
    inline void OLED_Clear(void);
    inline void OLED_Refresh_Gram(void);
    inline void OLED_ShowChar(uint8_t x,uint8_t y,uint8_t chr,uint8_t size,uint8_t mode);
    inline void OLED_ShowString(uint8_t x, uint8_t y, const uint8_t *p);
    inline void OLED_DrawPoint(uint8_t x,uint8_t y,uint8_t t);
    inline void OLED_Display_On(void);
    inline void OLED_Display_Off(void);
    inline uint32_t oled_pow(uint8_t m,uint8_t n);
    inline void OLED_ShowNumber(uint8_t x,uint8_t y,uint32_t num,uint8_t len,uint8_t size);
protected:
    
    uint8_t OLED_GRAM[128][8];
};

void Class_OLED::OLED_WR_Byte(uint8_t dat, uint8_t cmd)
{
    uint8_t i;
    if (cmd)
        OLED_RS_Set();
...
void Class_OLED::OLED_Refresh_Gram(void)
{
    uint8_t i, n;
    for (i = 0; i < 8; i++)
    {
...
void Class_OLED::OLED_Clear(void)
{
    uint8_t i, n;
    for (i = 0; i < 8; i++)
        for (n = 0; n < 128; n++)
...
void Class_OLED::OLED_ShowString(uint8_t x, uint8_t y, const uint8_t *p)
{
    uint8_t MAX_CHAR_POSX = 122, MAX_CHAR_POSY = 58;
    while (*p != '\0')
    {
        if (x > MAX_CHAR_POSX)
        {
...
void Class_OLED::OLED_ShowChar(uint8_t x,uint8_t y,uint8_t chr,uint8_t size,uint8_t mode)
{
    uint8_t temp, t, t1;
    uint8_t y0 = y;
    ...
    {
        if (size == 12)
            temp = oled_asc2_1206[chr][t]; // Invoke 1206 font   //调用1206字体
        else
            temp = oled_asc2_1608[chr][t]; // Invoke the 1608 font //调用1608字体
        for (t1 = 0; t1 < 8; t1++)
        {
            if (temp & 0x80)
...
void Class_OLED::OLED_DrawPoint(uint8_t x,uint8_t y,uint8_t t)
{
...
void Class_OLED::OLED_Display_On(void)
{
    OLED_WR_Byte(0X8D,OLED_CMD);  //SET DCDC command //SET DCDC命令
	OLED_WR_Byte(0X14,OLED_CMD);  //DCDC ON
	OLED_WR_Byte(0XAF,OLED_CMD);  //DISPLAY ON
}
void Class_OLED::OLED_Display_Off(void)
{
    OLED_WR_Byte(0X8D,OLED_CMD);  //SET DCDC command //SET DCDC命令
	OLED_WR_Byte(0X10,OLED_CMD);  //DCDC OFF
	OLED_WR_Byte(0XAE,OLED_CMD);  //DISPLAY OFF
}
uint32_t Class_OLED::oled_pow(uint8_t m, uint8_t n)
{
    uint32_t result = 1;
    while (n--)
        result *= m;
...
void Class_OLED::OLED_ShowNumber(uint8_t x,uint8_t y,uint32_t num,uint8_t len,uint8_t size)
{
    uint8_t t, temp;
    uint8_t enshow = 0;
    for (t = 0; t < len; t++)
