...
void UART_Init(UART_HandleTypeDef *huart, UART_Call_Back Callback_Function, uint16_t Rx_Buffer_Length)
{
    if (huart->Instance == USART1)
    {
	    ...
	    HAL_UART_Receive_DMA(&huart1,UART1_Manage_Object.Rx_Buffer,UART1_Manage_Object.Rx_Buffer_Length);
    }
    else if (huart->Instance == USART2)
    {
        ...
        //HAL_UART_Receive_IT(&huart2,UART2_Manage_Object.Rx_Buffer,UART2_Manage_Object.Rx_Buffer_Length);
			  __HAL_UART_ENABLE_IT(&huart2, UART_IT_IDLE); //使能空闲中断
	    HAL_UART_Receive_DMA(&huart2,UART2_Manage_Object.Rx_Buffer,UART2_Manage_Object.Rx_Buffer_Length);
    }
    else if (huart->Instance == USART3)
    {
	    ...
	    HAL_UART_Receive_DMA(&huart3,UART3_Manage_Object.Rx_Buffer,UART3_Manage_Object.Rx_Buffer_Length);
    }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        UART2_Manage_Object.Callback_Function(UART2_Manage_Object.Rx_Buffer, UART2_Manage_Object.Rx_Buffer_Length);
        HAL_UART_Receive_IT(&huart2,UART2_Manage_Object.Rx_Buffer,UART2_Manage_Object.Rx_Buffer_Length);
    }
}
